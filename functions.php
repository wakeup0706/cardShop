...
function cvf_demo_all_load_posts() {
    ...
    if(isset($_POST['page'])){
        ...
        $per_page = sanitize_text_field($_POST['per_page']);
        $search_key = sanitize_text_field($_POST['search_key']);
        $initial_key = sanitize_text_field($_POST['initial_key']);
        $summary_key = sanitize_text_field($_POST['summary_key']);
        $cur_page = $page;
        $page -= 1;
        $previous_btn = true;
        $next_btn = true;
        ...
        if($initial_key){
            $all_blog_posts = $wpdb->get_results($wpdb->prepare("
                SELECT * FROM " . $table_name . " WHERE post_type = 'all_terms' AND post_status = 'publish' AND (`post_title` LIKE '$initial_key%') ORDER BY $order LIMIT %d, %d", $start, $per_page ) );
            $count = $wpdb->get_var($wpdb->prepare("
                SELECT COUNT(ID) FROM " . $table_name . " WHERE post_type = 'all_terms' AND post_status = 'publish' AND (`post_title` LIKE '$initial_key%')", array() ) );
        } else if($summary_key){
            $all_blog_posts = $wpdb->get_results($wpdb->prepare("
                SELECT * FROM " . $table_name . " WHERE post_type = 'all_terms' AND post_status = 'publish' AND (`post_content` LIKE '%$summary_key%' OR `post_title` LIKE '%$summary_key%') ORDER BY $order LIMIT %d, %d", $start, $per_page ) );
            $count = $wpdb->get_var($wpdb->prepare("
                SELECT COUNT(ID) FROM " . $table_name . " WHERE post_type = 'all_terms' AND post_status = 'publish' AND (`post_content` LIKE '%$summary_key%' OR `post_title` LIKE '%$summary_key%')", array() ) );
        } else{
            $all_blog_posts = $wpdb->get_results($wpdb->prepare("
...
function cvf_sendEmail() {
    ...
    $name = $_POST['name'];
    $to = $_POST['to'];
    $subject = '視力向上可能性 判定結果のお知らせ';
    $message = '<!DOCTYPE html><html><meta charset="UTF-8"><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><body>';
    $message .= $content;
    $message .= '</body></html>';
    ...
    $headers .= "From: 眼育総研事務局 <info@469up.com>\r\n";

    if (wp_mail($to, $subject, $message, $headers)) {
        $message = "判定者名:". $name ."判定者メール:". $to . $message;
        wp_mail('simonscom1@gmail.com', $subject, $message, $headers);
    } else {
        echo "メールの送信に失敗しました。";
    }
    exit();
