...
function toStep18(){
  ...
  if(nearsightedRight == "近視" && refractiveRight > 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(nearsightedRight == "遠視" && refractiveRight < 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(nearsightedRight == "不明" && refractiveRight > 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(nearsightedLeft == "近視" && refractiveLeft > 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(nearsightedLeft == "遠視" && refractiveLeft < 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(nearsightedLeft == "不明" && refractiveLeft > 0){
    showToast("正確に入力してください！", "error");
    return;
  }
  if(refractiveRight && refractiveLeft && AstigmatismRight && AstigmatismLeft && CorrectedRight && CorrectedLeft)  handleSubmit();
  else  showToast("正確に入力してください！", "error");
}
function toStep19(){
  fullName = document.getElementById('fullName').value;
  if(fullName) handleSubmit();
  else  showToast("正確に入力してください！", "error");
}
