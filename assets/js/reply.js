...
$(document).ready(function() {
  ...
  if (CheckData_q10 == "知っている") {
    ...
    if (xR > 0 && yR > 0 && xL > 0 && yL > 0) {
      if (xR == xL && yR == yL) {
        fetch(php_var_uri + "/assets/matrix/" + yR + "-" + xR + "_RL.txt").then((res) => res.text())
        .then((text) => {
...
if(yearOfAge>=20){
    fetch(php_var_uri + "/assets/advice/advice00.txt")
      .then((res) => res.text())
      .then((text) => {
        CheckData_advice00 = text;
        $("#advice00").html(text);
      })
      .catch((e) => console.log(e));
  }
